...
class Ui(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.UpdateAffine_button.clicked.connect(self.refreshPlot)
        ...
        # Update Plot Button
        # self.Update_button.clicked.connect(self.refreshPlot)
        ...
        # self.ShowContinuumModel_check = QCheckBox("Show Continuum Models")
        # self.ShowContinuumModel_check.setChecked(False)
        # #self.PlotLayout.addWidget(self.ShowContinuumModel_check)
        ...
        # Ternary Plotting
        #self.AppendNormalization_button.clicked.connect(self.appendNormalizationToCSV)
        self.TernaryPlot_button.clicked.connect(self.plotTernaryDiagram)

        # self.AppendToCSV_check = QCheckBox("Append normalization to CSV")
        # self.SubtractionTab_layout.addWidget(self.AppendToCSV_check)
...
##        plt.show()
    ...
    def elementButtonClicked(self):
        ...
        def applyFitFlag(flag):
            ...
            # Only set the fit flag for the currently selected element (not both neutral and ion)
            # self.spectral.elemdata.els[self.elemIndex].user_fitflag = flag

            # Includes both neutral and ion if they are specified as fitted elements
    ...
    def plotMeasuredSpec(self, xshift=0., yshift=0):
        ...
        if hasattr(self, "ShowContinuumModel_check") and self.ShowContinuumModel_check.isChecked():
            ...
            try:
                ...
                mg_continuum_left = (485, 514)
                mg_continuum_right = (521, 550)
                na_continuum_left = (570, 585)
                na_continuum_right = (593, 608)

                # Accessing user-specified regions to subtract from
                
                ## WANT TO TRY THIS ##
                # mg_continuum_left = (self.MgLeft_spin1.value(), self.MgRight_spin1.value())
                # mg_continuum_right = (self.MgLeft2_spin.value(), self.MgRight2_spin.value())
                # na_continuum_left = (self.NaLeft_spin1.value(), self.NaRight_spin1.value())
                # na_continuum_right = (self.NaLeft2_spin.value(), self.NaRight2_spin.value())

                # mg_left1 = self.MgLeft_spin1.value()
                # mg_right1 = self.MgRight_spin1.value()
                # mg_left12 = self.MgLeft_spin2.value()
                # mg_right2 = self.MgRight_spin2.value()

                # na_left1 = self.NaLeft_spin1.value()
                # na_right1 = self.NaRight_spin1.value()
                # na_left2 = self.NaLeft_spin2.value()
                # na_right2 = self.NaRight_spin2.value()

                # # So ranges automatically update for user values
                # self.MgLeft_spin1.valueChanged.connect(self.refreshPlot)
                # self.MgRight_spin1.valueChanged.connect(self.refreshPlot)

                # self.MgLeft_spin2.valueChanged.connect(self.refreshPlot)
                # self.MgRight_spin2.valueChanged.connect(self.refreshPlot)

                # self.NaLeft_spin1.valueChanged.connect(self.refreshPlot)
                # self.NaRight_spin1.valueChanged.connect(self.refreshPlot)

                # self.NaLeft_spin2.valueChanged.connect(self.refreshPlot)
                # self.NaRight_spin2.valueChanged.connect(self.refreshPlot)

                fe_ranges = [(420, 455), (485, 514), (521, 550)]
                for fe_range in fe_ranges: # Want to see which regions are being averaged
                    self.Plot.addItem(
                ...
                #             pen=pg.mkPen(color=(255, 0, 0), width=2, style=QtCore.Qt.DashLine), name="Background Intensity around Mg")
                ...
                #             pen=pg.mkPen(color=(0, 0, 0), width=2, style=QtCore.Qt.DashLine), name="Background Intensity around Na")
    ...
    def savePlot(self):
        ...
        if hasattr(self, "ShowContinuumModel_check") and self.ShowContinuumModel_check.isChecked():
            ...
            try:
                ...
                #         color='red', linestyle='--', linewidth=2, label='Background Intensity around 518.2 nm')
                ...
                ax.plot(
                    wavelengths[mg_region_mask],
                    np.full(np.sum(mg_region_mask), mg_level),
                    color='red', linestyle=':', linewidth=2, label='Mg Background (median)'
                )
                ...
                #         color='black', linestyle='--', linewidth=2, label='Background Intensity around 589.2 nm')
                ...
                ax.plot(
                    wavelengths[na_region_mask],
                    np.full(np.sum(na_region_mask), na_level),
                    color='black', linestyle=':', linewidth=2, label='Na Background (median)'
                )
...
# Functions I made to improve MESS
    ...
    # Attempting new method of integrating the line intensities instead of taking them as single points
    def integrate_line(self, wavelengths, intensities, center, width=1.0):
        mask = (wavelengths >= center - width/2) & (wavelengths <= center + width/2)
        print("\nDEBUGGING")
        print(f"Integrating from {center-width/2} to {center+width/2} nm")
        print(f"Number of points in mask: {np.sum(mask)}")
        print(f"Wavelengths in mask: {wavelengths[mask]}")
        print(f"Intensities in mask: {intensities[mask]}")

        print("Wavelength range:", np.min(wavelengths), np.max(wavelengths))
        
        print("END DEBUGGING\n")

        return np.trapz(intensities[mask], wavelengths[mask])
    ...
    # Define the path to save the file
        ...
        # # For retrieving the event's date and time
        # dlg = QFileDialog()

        # dlg.selectedFiles()
        # file_path = os.path.split(dlg.selectedFiles()[0])[1]
                ...
                for element in self.spectral.elemdata.els: # Attempting to cap the for loop, could be what is crashing MESS
                    ...
                    if element.user_fitflag in [1, 2]:
                        fitting_elements.append({
                            ...
                            #'n_warm_total': getattr(element, 'n_warm_total', None),      # Total warm plasma density
                            'number_atoms': getattr(element, 'number_atoms', None),      # Number of atoms per element
        ...
        try:
            ...
            with open(file_name, 'w') as f:
                ...
                for elem in fitting_elements:
                    ...
                    #f.write(f"  n_warm_total: {elem['n_warm_total']}\n")      # <-- Add this line
                    f.write(f"  number_atoms: {elem['number_atoms']}\n")
    ...
    #     """
    #     fe_ranges = [(420, 455), (490, 515), (520, 550)]
    # #     for fe_range in fe_ranges: # Want to see which regions are being averaged
    # #         self.Plot.addItem(
    # #             pg.LinearRegionItem(values=fe_range, orientation='vertical', brush=(100, 100, 255, 50), movable=False)
    # # )
    ...
    # def calculateAverageFeIntensity(self, fe_ranges=[(430, 440), (490, 515), (520, 550)], top_n=15):
    #     """
    ...
    #             fe_mask = (wavelengths >= fe_range[0]) & (wavelengths <= fe_range[1])
    #             fe_intensities = intensities[fe_mask]
    #             all_fe_intensities.extend(fe_intensities.tolist())
    #             average_intensity = np.mean(fe_intensities) if len(fe_intensities) > 0 else 0.0
    #             if float(average_intensity) == 0.0:
    #                 print(f"No intensities found in range {fe_range}. Skipping this range.")
    #                 continue
    ...
    #         # Take the top N highest intensity points across all ranges
    #         if all_fe_intensities:
    #             top_points = sorted(all_fe_intensities, reverse=True)[:top_n]
    #             top_n_average = np.mean(top_points)
    #             print(f"Top {top_n} Fe intensities: {top_points}")
    #             print(f"Average of top {top_n} Fe intensities: {top_n_average:.6f}")
    #         else:
    #             top_points = []
    #             top_n_average = 0.0

    #         return fe_averages, top_points, top_n_average
    #         # return top_n_average
    #     except Exception as e:
    #         print(f"Error calculating average Fe intensity: {e}")
    #         return None, [], 0.0
        

    def calculateAverageFeIntensity(self, fe_ranges=[(430, 440), (485, 513), (522, 550)], top_n=15, exclusion_nm=1.0):
        """
        Calculate the average of the highest N intensity points across all Fe ranges,
    ...
    def subtractContinuum(self):
        ...
        try:
            ...
            intensity_sum_3 = intensity_dict['Mg_518nm'] + intensity_dict['Na_589nm'] + top_n_average
            
            print(f"Sum of intensities (Mg + Na + Fe): {intensity_sum_3:.6f}")
    ...
    def saveSubtractionInfo(self):
        ...
        # Only prompted if the save path is not set
        save_path = self.SavePath_edit.text()
        if not save_path:
            print("Save path is not set. Please choose a save path.")
            return

        file_name = os.path.join(save_path, "saved_subtraction_info_topN.txt")
        try:
            with open(file_name, 'w') as f:
                ...
                f.write(f"\nTop N Fe Intensity: {top_n_average:.6f}\n")
                ...
                f.write(f"Sum of intensities (Mg + Na + Fe): {intensity_dict['Mg_518nm'] + intensity_dict['Na_589nm'] + top_n_average:.6f}\n\n")
            ...
            print("Reached finally block in saveSubtractionInfo.")
            if hasattr(self, "AppendToCSV_check"):
                print('Checkbox exists, checked:', self.AppendToCSV_check.isChecked())
            if hasattr(self, "AppendToCSV_check") and self.AppendToCSV_check.isChecked():
                self.appendNormalizationToCSV()
                print("Appending normalization to CSV file.")


    ## For the fifth tab used for background subtraction ; will keep functions only here eventually

    # Update with max fe value
    def subtractContinuum_bg(self):
        ...
        try:
            ...
            # intensity_sum = intensity_dict['Mg_518nm'] + intensity_dict['Na_589nm'] + fe_average_value
            intensity_sum = intensity_dict['Mg_518nm'] + intensity_dict['Na_589nm'] + top_n_average
            ...
            # norm_intensities['Fe'] = round(fe_average_value / intensity_sum, 6) if intensity_sum != 0 else 0.0
            norm_intensities['Fe'] = round(top_n_average / intensity_sum, 6) if intensity_sum != 0 else 0.0
...
# Functions for Ternary Plotting
    def appendNormalizationToCSV(self):
        ...
        # Get normalized intensities from the latest subtraction
        norm = getattr(self.spectral, 'normalized_intensities', None)
        if not norm or not all(k in norm for k in ['Fe', 'Mg', 'Na']):
            print("No normalized intensities available. Run continuum subtraction first.")
            return
        ...
        # print(f"Event name parts: {parts}")
        # self.event_date = parts[0]
        # self.event_time = parts[1] if len(parts) > 1 else ""

        # Define date/time info
        date = self.event_date if hasattr(self, 'event_date') and self.event_date else ""
        time = self.event_time if hasattr(self, 'event_time') and self.event_time else ""
        ...
        if file_dialog.exec():
            ...
            print("No file selected for appending normalization values.")
            return
        ...
        print(f"Appending row: Fe={norm['Fe']}, Mg={norm['Mg']}, Na={norm['Na']}, Date/Time={date}_{time}")
        ...
        try:
            ...
            print(f"Appended normalization values to {file_path}")
        except Exception as e:
            print(f"Error appending to CSV: {e}")

    def plotTernaryDiagram(self):
        """
